...
export default function VictimTab({ caseId, refetch }) {
  ...
  const loadVictims = () => listVictims(caseId).then(setVictims).catch(() => setVictims([]))
  ...
  return (
    <>
      <div className="card mb-4 p-4 sm:p-6">
        <h3 className="mb-1 text-base font-semibold text-slate-800">{editingId ? 'Edit Victim' : 'Add Victim'}</h3>
      ...
      <div className="card p-4 sm:p-6">
        <h3 className="mb-4 text-base font-semibold text-slate-800">Victims ({victims.length})</h3>
        {victims.length === 0 ? (
            ...
            {victims.map((v) => (
              <div key={v.id} className="flex items-center justify-between rounded-xl border border-border px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{v.age} yrs · {v.gender} {v.mobile ? `· ${v.mobile}` : ''}</p>
                </div>
