...
export default function MedicalTab({ caseId }) {
  ...
  if (victims.length === 0) {
    return <div className="card p-6"><p className="text-sm text-slate-400">No victims on this case yet.</p></div>
  }
  ...
  return (
    <>
      ...
      <div className="card mb-4 p-4 sm:p-6">
        <h4 className="mb-4 text-sm font-semibold text-slate-700">{editingId ? 'Edit Medical Record' : 'Add Medical Record'}</h4>
      ...
      <div className="card p-4 sm:p-6">
        <h4 className="mb-4 text-sm font-semibold text-slate-700">Records ({records.length})</h4>
        {records.length === 0 ? (
            ...
            {records.map((r) => (
              <div key={r.id} className="flex items-center justify-between rounded-xl border border-border px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{r.mlcNumber ? `MLC: ${r.mlcNumber} · ` : ''}{r.death ? 'Deceased' : 'Survived'}</p>
                </div>
                <button onClick={() => handleEdit(r)} className="rounded-lg p-2 text-slate-400 hover:bg-amber-50 hover:text-amber-600"><Pencil size={16} /></button>
              </div>
            ))}
          </div>
        )}
