...
export default function Step3VehicleInsurance({ wizard }) {
  ...
  return (
    <>
      <div className="card mb-4 p-4 sm:p-6">
        ...
        <p className="mb-4 text-sm text-slate-400">A case can involve more than one vehicle — add each one separately.</p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Input label="Policy Holder" name="policyHolder" placeholder={form.ownerName || 'Defaults to owner name'} value={form.policyHolder} onChange={handleChange} />
              ...
              <div key={v.id} className="flex items-center justify-between rounded-xl border border-border px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {v.vehicleType} · {v.ownerName} {v.insuranceDetails?.[0]?.insuranceCompany ? `· ${v.insuranceDetails[0].insuranceCompany}` : ''}
                  </p>
                </div>
